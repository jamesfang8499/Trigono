...
\[\left\{x\mid x=-\frac{\pi}{4}+2k\pi \quad\text{ 或 }\quad x=\frac{5\pi}{4}+ 2k\pi,\; k\in\Z\right\}\]
...
\begin{enumerate}[(1)]
    ...
    \item $2\cos\left(\frac{x}{3}+45^{\circ}\right)=1$
    \item $\tan 2x-\sqrt{3}=0$
...
\begin{minipage}{.45\textwidth}
\begin{remark}
这里我们运用了一系列同解变形的步骤（关键的是哪几步？），这就保证了最后得到的$x$是原方程的解。下面的解法先去分母（会增解呢？还是丢解？你能讲清楚解方程时增解或丢解的最根本原由吗？）。
\end{remark}
\end{minipage}
