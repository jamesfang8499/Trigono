...
\begin{scope}
    ...
    \draw[->](0,-1.5)node[below]{$\sin\alpha$与$\cos\alpha$}--(0,1.5)node[left]{$y$};
...
知$\alpha$的一个函数值，求$\alpha$的三角函数式的值的问
    ...
    由于$\cot\alpha=-3$，$\alpha$是II或IV象限中的角，而所求各式都是$\sin\alpha$、$\cos\alpha$的“齐次式”，抓住这一结构特征，问题就不难解决了。
